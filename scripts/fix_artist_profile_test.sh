#!/bin/bash
# Script to fix the artist profile service test in the ARTbeat project
# Created: June 15, 2025

echo "Starting fix for artist profile service test..."

# Set file paths
SRC_DIR="/Users/kristybock/artbeat/packages/artbeat_artist/test/services"
BACKUP_DIR="/Users/kristybock/artbeat/test_backups"
ORIGINAL_FILE="$SRC_DIR/testable_artist_profile_service_test.dart"
FIXED_FILE="$SRC_DIR/testable_artist_profile_service_fixed.dart"
BACKUP_FILE="$BACKUP_DIR/testable_artist_profile_service_test.bak.$(date +%Y%m%d%H%M%S)"

# Create backup directory if it doesn't exist
mkdir -p "$BACKUP_DIR"

# Backup the original file
echo "Backing up original artist profile service test file to $BACKUP_FILE"
cp "$ORIGINAL_FILE" "$BACKUP_FILE"

# Create the fixed file with proper implementation
echo "Creating fixed artist profile service test file..."
  ...
  group('ArtistProfileService Tests', () {
    ...
    test(
      ...
      await artistProfileService.updateArtistProfile(
        ...
        socialLinks: {'instagram': 'updated_handle', 'facebook': 'new_facebook'},
      );
    ...
    test('deleteArtistProfile should remove profile from database', () async {
      // Arrange - Create profile
      final profileId = 'profile-to-delete';
      await fakeFirestore.collection('artistProfiles').doc(profileId).set({
        'userId': testUserId,
      ...
      // Act
      await artistProfileService.deleteArtistProfile(profileId);
    ...
    test('deleteArtistProfile should throw exception for non-existent profile',
        () async {
      // Act & Assert
      expect(
        () => artistProfileService.deleteArtistProfile('non-existent-profile'),
        throwsException,
      );
    });
...
echo "Fixed test file created at $FIXED_FILE"
echo "To use the fixed version, you can run:"

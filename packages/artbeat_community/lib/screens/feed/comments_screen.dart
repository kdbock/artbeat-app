...
class _CommentsScreenState extends State<CommentsScreen> {
  ...
  Future<void> _addComment() async {
    ...
    try {
      ...
      debugPrint('👤 Comment user model: ${userModel.fullName}');
      debugPrint(
        '🖼️ Comment user avatar URL: "${userModel.profileImageUrl ?? 'null'}"',
      );
      debugPrint(
        '🔍 userModel.profileImageUrl is null: ${userModel.profileImageUrl == null}',
      );
      debugPrint(
        '🔍 userModel.profileImageUrl is empty: ${userModel.profileImageUrl?.isEmpty ?? 'N/A'}',
      );

      // Get the avatar URL, but don't default to empty string if null
      final avatarUrl = userModel.profileImageUrl;
      debugPrint('🎯 Final avatarUrl for comment: "${avatarUrl ?? 'null'}"');
      ...
      final commentDoc = {
        ...
        'userAvatarUrl':
            avatarUrl ??
            '', // Keep as empty string for Firestore, but we'll handle this in UI
      ...
      setState(() {
        ...
        _replyingTo = null;
        _commentType =
            'Appreciation'; // Reset to default value that exists in the list
      });
